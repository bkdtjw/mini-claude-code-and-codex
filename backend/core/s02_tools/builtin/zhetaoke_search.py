...
from backend.core.s02_tools.commerce_tool_guidance import ZHETAOKE_NO_RESULT_NOTE, ZHETAOKE_TOOL_NOTE
...
def create_zhetaoke_taobao_search_tool(
    ...
    definition = ToolDefinition(
        ...
        parameters=ToolParameterSchema(
            properties={
                "q": {"type": "string", "description": "必填，关键词、店铺名或商品明文链接"},
...
def _format_report(keyword: str, products: list[ZhetaokeSearchProduct]) -> str:
    ...
    if not products:
        return f'折淘客淘宝搜索: "{label}" {ZHETAOKE_NO_RESULT_NOTE}'
