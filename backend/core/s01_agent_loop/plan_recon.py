...
探索预算：
- 优先读取用户明确提到的文件和它们的一层直接调用方。
- 同一个文件原则上不要重复读取；如需查看不同片段或上下文，可以再次读取，但必须有明确新目的。
- 已经确认的事实不要再次验证，避免无新增信息的读取、搜索或重试。
- 对只读验证类任务，读取关键文件后应立即收口输出 JSON。
- 如果收到收口或反思提示，仍必须输出符合下方格式的 JSON。
