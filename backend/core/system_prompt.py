...
def build_system_prompt(workspace: str | None = None) -> str:
    ...
    if os_name == "Windows":
        shell_info = "cmd.exe。使用 dir（不要用 ls）、type（不要用 cat）、cd、findstr 等 Windows 命令。"
        command_rule = "绝对不要使用 Linux 命令（pwd、ls、cat、grep），只用 Windows 命令（dir、type、cd、findstr）。"
    else:
    ...
    parts = [
        ...
        "如果连续 3 次工具调用失败，停止继续调用工具，直接向用户解释失败原因、当前限制和下一步建议。",
    ]
    if workspace:
