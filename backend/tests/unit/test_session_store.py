...
async def test_session_store_crud_roundtrip() -> None:
    ...
    try:
        created = await store.create(session, title="初始标题", workspace="C:/demo")
        ...
        assert [item.content for item in saved_messages] == ["查看目录", "我来查看"]
