...
logger = get_logger(component="websocket")
router = APIRouter()
...
class ConnectionManager:
    ...
    async def disconnect(
        ...
        *,
        ...
        # 先做纯同步、不可失败的清理，确保订阅任务与连接引用一定被释放。
        ...
        if loop is not None:
            ...
            # 无在跑任务时回收 loop（落库已在上方完成），避免断连后 loop 常驻内存。
            # 有任务在跑则保留：后台 run 仍依赖它，交由其 done_callback 回收。
...
async def ws_endpoint(websocket: WebSocket, session_id: str) -> None:
    ...
    try:
        ...
        pass  # 正常断开，跳出循环即可，清理统一交给 finally。
    except Exception as exc:  # noqa: BLE001
